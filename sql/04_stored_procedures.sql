-- RideSync (Project 2) — Step 3 Workflow 1: atomic booking
--
-- One transaction:
--   1) debit the rider wallet (fare held as escrow)
--   2) audit trigger writes a DEBIT row into wallet_audit_logs
--   3) insert a REQUESTED trip
-- CHECK failure (e.g. wallet would go negative) or an active-trip unique
-- violation aborts the whole transaction — no leftover debit or trip.
--
